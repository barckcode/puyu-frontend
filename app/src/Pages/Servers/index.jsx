            ...
            {selectedCloud && regions.length > 0 ? (
                <form className="mt-8 p-8 flex flex-col items-center justify-center text-indigo-900">
                    ...
                    <div className="mt-8 flex justify-between w-1/2">
                        <button type="button" className="p-2 w-12 flex items-center justify-center bg-gray-300 rounded-lg"><FaArrowLeft /></button>
                        <button type="button" className="p-2 w-32 text-center bg-gray-300 rounded-lg" onClick={openModal}>Summary</button>
                    </div>
                </form>
            ) : selectedCloud ? (
                                ...
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                                    ...
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500">
                                            Cloud: {selectedCloud?.name || 'Select a cloud'}
                                            <br />
                                            Name: {serverName}
                                            <br />
                                            Region: {selectedRegion?.name || 'Select a region'}
                                            <br />
                                            Distribution: {selectedDistribution?.distribution} {selectedDistribution?.version}
                                            <br />
                                            Instance Type: {selectedInstanceType?.cpu} / {selectedInstanceType?.memory}
                                            <br />
                                            Storage: {selectedStorage?.size} GiB
                                        </p>
                                    </div>
                                    <div className="mt-4">
                                        <button
